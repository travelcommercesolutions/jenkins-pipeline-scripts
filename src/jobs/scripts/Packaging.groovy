    ...
    */
    ...
    def static createReleaseArtifact(context, version, webProject, zipArtifact, websiteDir)
    {
        ...
        else
        {
            ...
            context.powershell script: "Copy-Item -Path ${context.env.WORKSPACE}\\${websiteDir}\\App_Data\\* -Destination ${websitePath}\\App_Data -Recurse -Force"
        }
    ...
    def static startAnalyzer(context)
    {
        ...
        context.withSonarQubeEnv('TCS Sonar Server') {
            ...
            if(Utilities.isPullRequest(context)){
                context.bat "dotnet sonarscanner begin /n:\"${fullJobName}\" /k:\"${fullJobName}\" /d:sonar.github.oauth=${context.env.GITHUB_TOKEN} /d:sonar.analysis.mode=preview /d:sonar.github.pullRequest=\"${prNumber}\" /d:sonar.github.repository=${orgName}/${repoName} /d:sonar.host.url=%SONAR_HOST_URL% /d:sonar.login=%SONAR_AUTH_TOKEN% /d:sonar.cs.${coverageReportType}.reportsPaths=\"${coverageFolder}\\VisualStudio.Unit.coveragexml\""
            }
            else{
                // Due to SONARMSBRU-307 value of sonar.host.url and credentials should be passed on command line
                context.bat "dotnet sonarscanner begin /n:\"${fullJobName}\" /k:\"${fullJobName}\" /d:sonar.host.url=%SONAR_HOST_URL% /d:sonar.login=%SONAR_AUTH_TOKEN% /d:sonar.cs.${coverageReportType}.reportsPaths=\"${coverageFolder}\\VisualStudio.Unit.coveragexml\""
            }
        }        
    }
	...
	def static publishRelease(context, version, releaseNotes, branch)
	{
		...
		context.dir(packagesDir)
		{
			...
			if (artifacts.size() > 0) {
				...
				{
					packageUrl = Packaging.publishGithubRelease(context, releaseTag, releaseNotes, additionalParameters, artifacts[i])
				}
			}
		}
	...
	def static publishGithubRelease(context, releaseTag, releaseNotes, additionalParameters, artifact)   
	{
		...
		context.bat "${context.env.Utils}\\github-release release ${additionalParameters} --user $REPO_ORG --security-token ${context.env.GITHUT_TOKEN_TMP} --repo $REPO_NAME --tag ${releaseTag} --description \"${releaseNotes}\""
		context.bat "${context.env.Utils}\\github-release upload --user $REPO_ORG --security-token ${context.env.GITHUT_TOKEN_TMP} --repo $REPO_NAME --tag ${releaseTag} --name \"${artifact}\" --file \"${artifact}\""
